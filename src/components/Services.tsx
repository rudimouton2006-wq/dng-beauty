...
import { ArrowRight, Clock, ChevronLeft, ChevronRight, Award } from "lucide-react";
...
const EXTENSIONS = [
  ...
  { 
    ...
    images: ["/images/wispy-spikes-new.jpg"], 
...
const ADD_ONS = [
  { id: "a1", name: "Brow Shape", price: "R100", duration: "30 Min", desc: "Precision waxing and tweezing to sculpt your arches perfectly to your bone structure.", img: "/images/brow-shape-new.jpg" },
  { id: "a_tint", name: "Brow Tint", price: "R100", duration: "30 Min", desc: "Custom color-matching to define and fill the brow architecture for a bolder, polished look.", img: null },
  { id: "a2", name: "Brow Shape + Tint", price: "R200", duration: "1 Hour", desc: "Sculpting paired with custom color-matching to define and fill the brow architecture.", img: "/images/brow-shape-tint.jpg" },
  { id: "a_lip", name: "Lip Wax", price: "R100", duration: "15 Min", desc: "Gentle and precise upper lip waxing for a smooth, flawless finish.", img: null },
...
const TRAINING = [
  { 
    id: "m-class", 
    name: "2-Day Lash Masterclass", 
    duration: "2 Days", 
    desc: "An intensive, hands-on masterclass designed to elevate your lash artistry. Learn advanced techniques, bespoke mapping, and safe application. Includes an exclusive starter kit, live model practice, and your official Certificate of Completion.",
    images: ["/images/classic-extensions.jpg"], 
    prices: [ { label: "Total Investment", price: "R3500" } ]
  }
];

const CATEGORIES = ["Extensions", "Clusters", "Add-Ons", "Training"];
  ...
  return (
    <main className="bg-[#FAF9F6] min-h-screen font-sans text-[#1A1A1A] pt-32 pb-20">
      <div className="max-w-7xl mx-auto px-6 lg:px-20">
        ...
        <motion.div initial="hidden" animate="visible" variants={fadeUp} className="flex flex-wrap justify-center gap-4 sm:gap-6 mb-16 border-b border-gray-200 pb-4">
        ...
        <AnimatePresence mode="wait">
              ...
              {EXTENSIONS.map((service) => (
                  <div key={service.id} className="flex flex-col group">
                      ...
                      <div className="flex flex-col flex-grow px-2">
                          ...
                          <button onClick={() => setPage("booking")} className="w-full py-4 border border-[#1A1A1A] text-[#1A1A1A] font-bold tracking-widest uppercase text-[10px] hover:bg-[#1A1A1A] hover:text-white transition-colors duration-300 rounded-sm mt-auto">
                              Book Appointment
                          </button>
                      </div>
                  </div>
            ...
            <motion.div key="addons" initial="hidden" animate="visible" exit="exit" variants={fadeUp} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
              {ADD_ONS.map((addon) => (
                  <div key={addon.id} className="flex flex-col group">
                      <div className="aspect-square overflow-hidden bg-gray-50 relative mb-6 rounded-sm flex items-center justify-center border border-gray-100 group-hover:border-gray-200 transition-colors">
                          {addon.img ? (
                              ...
                              <div className="text-center px-4">
                                  <span className="text-sm font-light text-gray-400 uppercase tracking-widest">{addon.name}</span>
                              </div>
                          )}
          ...
          {activeCategory === "Training" && (
            <motion.div key="training" initial="hidden" animate="visible" exit="exit" variants={fadeUp} className="max-w-4xl mx-auto">
              {TRAINING.map((service) => (
                  <div key={service.id} className="flex flex-col md:flex-row gap-10 bg-white border border-gray-200 rounded-sm p-8 shadow-sm">
                      <div className="w-full md:w-1/2 aspect-square md:aspect-[4/5] overflow-hidden bg-gray-100 relative rounded-sm">
                          <ImageCarousel images={service.images} />
                          <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-md px-3 py-1.5 rounded-sm flex items-center gap-1.5 shadow-sm">
                              <Clock size={10} className="text-[#1A1A1A]" />
                              <span className="text-[9px] font-bold uppercase tracking-widest text-[#1A1A1A]">{service.duration}</span>
                      ...
                      <div className="w-full md:w-1/2 flex flex-col justify-center">
                          <div className="flex items-center gap-2 mb-3">
                              ...
                              <span className="text-[10px] font-bold tracking-widest uppercase text-gray-400">Certification Course</span>
                          </div>
                          <h3 className="text-3xl font-light tracking-tight text-[#1A1A1A] mb-4">{service.name}</h3>
                          <p className="text-gray-500 font-light text-sm leading-relaxed mb-8">{service.desc}</p>
                          
                          <div className="space-y-4 mb-8 bg-gray-50 p-6 rounded-sm border border-gray-100">
                            {service.prices.map((p, i) => (
                              <div key={i} className="flex justify-between items-center">
                                <span className="text-xs font-bold uppercase tracking-widest text-gray-500">{p.label}</span>
                          ...
                          <button onClick={() => setPage("booking")} className="w-full py-5 bg-[#1A1A1A] text-white font-bold tracking-widest uppercase text-[10px] hover:bg-gray-800 transition-colors duration-300 rounded-sm mt-auto">
                              Enroll Now
                          </button>
                      </div>
                  </div>
